...
<p>
	...
	<div id="col2">
	    <div id="aliens_arrival_div">
	        <p id="ard_title">Active Battles!</p>
			...
			while($row = $result->fetch_row()) {
				...
				echo "\n\t\t" . '<div class="battle_info_div2">' .
					 ...
					 "\n\t\t\t" . '<a class="attacker_name" href="country?country_id=' . $attacker_id . '" target="_blank">' . 
									$attacker_country_name . '</a>' . '<img class="attacker_flag" src="../country_flags/' . $attacker_flag . '" alt="' . $attacker_country_name . '">' .
					 "\n\t\t\t" . '<a class="defender_name" href="country?country_id=' . $defender_id . '" target="_blank">' . 
									$defender_country_name . '</a>' . '<img class="defender_flag" src="../country_flags/' . $defender_flag . '" alt="' . $defender_flag . '">';
		...
		<?php
			...
			/* Aliens' arrival  
			$date1 = new DateTime(date('Y-m-d G:i:s', strtotime("2021-06-14 14:00:00")));
...
*/
	...
	<div id="col3">
	    <?php
	    	...
	    	/* Admin Blog */
			$query = "SELECT post_id, title FROM blog_posts WHERE blog_id = 75940216220527771 AND
					  date > DATE_SUB(CURRENT_DATE, INTERVAL 20 DAY) ORDER BY date DESC, time DESC LIMIT 1";
			$result = $conn->query($query);
			if($result->num_rows == 1) {
				$row = $result->fetch_row();
	   ...
	   <?php 
	    ...
	    "\t\t\t\t" . '<a id="latest_posts_div_head" href="latest_posts"><i class="fa fa-file-text-o" aria-hidden="true"></i> ' .
			...
			$query = "SELECT post_id, title, post, date, time, blog_image, blog_name, ub.blog_id, user_name, ub.user_id,
					  ...
					  WHERE ub.blog_id IN 
					 (SELECT blog_id FROM blog_subscribers) 
