			<div id="user">
				...
				<div id="user_info">
					...
					<abbr title="region">
						<div id="citizenship">
							...
							<?php
								$sql = "SELECT region_name, region_id, FROM regions 
										WHERE region_id = (SELECT loc_region FROM user_profile 
										WHERE user_id = '$user_id')";
								$result = mysqli_query($conn, $sql);
								$row = mysqli_fetch_row($result);
								list($region_name, $loc_region) = $row;
								echo "\n\t\t\t\t\t" .  '<a href="region_info?region_id=' . $loc_region . '">' . $region_name . '</a>';
							?>
				...
				$change_time = date('Y-m-d H:i:s', strtotime("$date 00:00:00 + 1 DAY"));
