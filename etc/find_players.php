<?php
	...
	//query
	if($country_id == 0) {
		$query = "SELECT  experience, u.user_id, user_name, user_image, flag, DATEDIFF(CURRENT_DATE, register_date)
				  ...
				  AND DATEDIFF(CURRENT_DATE, register_date) <= '$to_days' ORDER BY experience DESC LIMIT 50";
	}
	else {
		$query = "SELECT experience, u.user_id, user_name, user_image, flag, DATEDIFF(CURRENT_DATE, register_date)  
				  FROM users u, user_profile up, country c
				  WHERE up.user_id = u.user_id AND c.country_id = up.citizenship AND
				  u.user_name LIKE '$player_name' AND DATEDIFF(CURRENT_DATE, register_date) >= '$from_days' 
				  AND DATEDIFF(CURRENT_DATE, register_date) <= '$to_days' AND citizenship = '$country_id'  ORDER BY experience DESC LIMIT 50";
	}
	$result = $conn->query($query);
	$x = 0;
	while($row = $result->fetch_row()) {
		list($experience, $user_id, $user_name, $user_image, $flag, $days_in_game ) = $row;
		
		$players[$x]['experience'] = $experience;
